...
| **K8S MCP Server** | Kubernetes MCP server for cluster resource access | `lightspeedCore.enabled=true` |
...
By default OLS is configured to use the OpenAI provider with the gpt-4o-mini model. To use a different provider or model:
...
By default Lightspeed Core is configured to use the OpenAI provider with the gpt-4o-mini model. To use a different provider or model:
...
**Note:** ClusterRoles and ClusterRoleBindings are cluster-scoped and will be deleted. Secrets and ConfigMaps will also be removed.
